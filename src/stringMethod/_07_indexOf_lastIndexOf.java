package stringMethod;

public class _07_indexOf_lastIndexOf {

    public static void main(String[] args) {
        /*

        Method indexOf() and lastIndexOf() ->  are used to find the index or last index of some char or String
        values in another String

        -They are non-static methods and called with another String object
        -They are return type, and they return int as index
        -They take either String or char as arguments

        NOTE: if the given char or String does not exist, then they return -1
        NOTE: if you are looking for an index of String, and it exists, it will return the first
        index of found match
         */
       //  EXAMPLE: sentence.indexOf("Chicago"); // 7



///                       0......7
       String sentence = "I like Chicago and Miami more than any other cities";

        System.out.println(sentence.indexOf('C')); // 7
        System.out.println(sentence.indexOf('c')); // 10
        System.out.println(sentence.lastIndexOf('c')); // 45  // "c"ities 45

        sentence. indexOf("Chicago"); // 7  numara caracterele de la 0 la Chicago
        sentence.indexOf("Miami"); // 19
        sentence. indexOf("chicago"); // -1  daca nu exista
        sentence. indexOf("Florida"); // -1
        System.out.println(sentence. indexOf("chicago")); // -1 print -1 if the character does NOt exist
        System.out.println(sentence. indexOf("Florida")); // -1

        System.out.println( sentence.indexOf('c', 15)); //45 //  skip first 50 characters and then check for 'c'
    }
}
